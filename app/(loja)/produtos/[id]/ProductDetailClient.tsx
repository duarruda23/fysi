...
import type { Peca, Tamanho, VariacaoPeca, Avaliacao } from "@/lib/types";
...
export default function ProductDetailClient({ pecaInicial }: { pecaInicial: Peca }) {
  const router = useRouter();
  const { pecas, addToCart, promocoes, inscreverVip, clienteLogado } = useStore();

  // Usa a peça já carregada pelo store (fica em dia com estoque/preço) e,
  // enquanto o store ainda não terminou de buscar, cai pro dado vindo do servidor.
  const peca = useMemo(() => {
    return pecas.find((p) => p.id === pecaInicial.id) ?? pecaInicial;
  }, [pecas, pecaInicial]);
