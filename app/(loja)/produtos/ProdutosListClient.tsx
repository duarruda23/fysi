...
function CatalogContent({ pecasIniciais }: { pecasIniciais: Peca[] }) {
  const { pecas: pecasStore } = useStore();
  // Enquanto o store do cliente ainda não terminou de buscar, usa o dado vindo do servidor.
  const pecas = pecasStore.length > 0 ? pecasStore : pecasIniciais;
...
export default function ProdutosListClient({ pecasIniciais }: { pecasIniciais: Peca[] }) {
  return (
    <React.Suspense fallback={<div className="py-20 text-center text-sm text-coal/50 font-serif">Carregando catálogo Fysi...</div>}>
      <CatalogContent pecasIniciais={pecasIniciais} />
    </React.Suspense>
  );
}
